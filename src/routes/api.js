...
router.post('/user', async (req, res) => {
    logger.info({ req, res, message: `Creating user with username: ${ req.body.username }` });

    try {
        const newUser = await user.post(req.body);
        logger.info({ req, res, message: `Created new user with username: ${ newUser.username }` });
        res.status(200).send({ error: null, response: newUser });
    } catch (err) {
        logger.error({ req, res, message: `Caught error while creating new user with username: ${ req.body.username }`, err });
